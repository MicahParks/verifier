...
func main() {
	...
	err := json.Unmarshal(jwksJSON, &marshal)
	if err != nil {
		log.Fatalf("Failed to unmarshal the JWK Set.\nError: %s", err)
	}

	// Convert this type to jwkset.Storage.
	jwks, err := marshal.ToStorage()
	if err != nil {
		log.Fatalf("Failed to create a JWK Set storage.\nError: %s", err)
	}
	...
	k, err := verifier.New(options)
	if err != nil {
		log.Fatalf("Failed to create a verifier.JWKSetVerifier.\nError: %s", err)
	}

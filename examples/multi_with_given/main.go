...
func main() {
	...
	jwks, err := verifier.NewDefault([]string{s1.URL, s2.URL})
	if err != nil {
	...
	if err != nil {
		log.Fatalf("Failed to create signer.\nError: %s", err)
	}
	token, err := jwt.NewBuilder(signer, jwt.WithKeyID(givenKeyID)).Build(jwt.RegisteredClaims{})
	if err != nil {
		log.Fatalf("Failed to build JWT.\nError: %s", err)
	}
	signed := token.Bytes()
